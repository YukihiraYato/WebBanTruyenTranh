...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    @Query("""
            ...
            WHERE oi.order.status = 'DELIVERED'
                    and year (oi.order.deliveredDate) = :year
                    and month (oi.order.deliveredDate) = :month
            GROUP BY oi.book.bookId, oi.book.title
            ORDER BY SUM(oi.quantity) DESC
            """)
    List<Object[]> findTopSellingBooks(@Param("year") Integer year, @Param("month") Integer month);
}
