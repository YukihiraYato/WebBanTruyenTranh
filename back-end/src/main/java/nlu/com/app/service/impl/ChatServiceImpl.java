...
public class ChatServiceImpl implements IChatService {
    ...
    public String reply(Long userId) {
        ...
        try {
            String formatted = """
                    ...
                    Nếu data được cung cấp bị null hoặc rỗng, hãy thông báo người dùng dữ liệu mà người dùng tìm không có trong hệ thống, không được tự suy đoán lan man hoặc giới thiệu sách khác.
                    Nếu tin nhắn mới nhất của User nói cảm ơn, khen ngợi, chào hỏi, trả lời xã giao, thì hãy đáp lại họ 1 cách lịch sự
    ...
    public IntentResponseDTO detectIntent(Long userId) {
        ...
        String prompt = """
                ...
                3. Nếu user hỏi về tên truyện cụ thể → luôn map vào books.title
                4. Chỉ trả về JSON thuần, không giải thích thêm.
                5. Nếu các fields của 1 intent không tìm thấy value trong message user thì hãy gán giá trị null. 
                6. "Chỉ trả về JSON thuần, không thêm bất cứ ký tự nào khác, trừ trường hợp nếu trong message user không tìm thấy bất kì intent nào thì trả thẳng null"
                7.  Nếu tin nhắn mới nhất của User nói cảm ơn, khen ngợi, chào hỏi, trả lời xã giao, trả về null.
                Câu hỏi của user: "%s"
                """.formatted(message);
