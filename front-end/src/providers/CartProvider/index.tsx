...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const increaseItem = async (bookId: string, quantity: number, typePurchase: string): Promise<CartResult> => {
    try {
      const result = await addToCart(bookId, quantity, typePurchase);
      fetchCart();
      return result;
    } catch (err) {
  ...
  const decreaseItem = async (bookId: string, quantity: number, typePurchase: string): Promise<CartResult> => {
    try {
      const result = await addToCart(bookId, quantity, typePurchase);
      fetchCart();
      return result;
