...
import {getRefundRequestsByOrderId} from "~/api/user/refund";
function onBuyAgain() {
...
export default function RefundItem({
  ...
  function handleClick() {
    ...
    navigate(`/profileUser/orders/view/order_id/${orderId}`);
  }
  ...
  useEffect(() => {
    const fetchRefundRequests = async () => {
      try {
        const data = await getRefundRequestsByOrderId(parseInt(orderId));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Grid2>
          {/* Hiện status của hóa đơn, auto là delivery */}
          <Typography variant="body2" color="text.secondary">
           {refundRequest ? refundRequest.status : ''}
          </Typography>
        </Grid2>
      </Grid2>
      ...
      >
        ...
        <Grid2>
          <Stack direction="row" spacing={1}>
            ...
            {status === "DELIVERED" && refundRequest === undefined  ?(
              <Button
                variant="outlined"
                color="error"
                onClick={(e) => {
     ...
     {confirmCancel && (
              <CustomSnackbar
                ...
                severity="warning"
                message="Bạn có chắc chắn muốn hoàn lại đơn hàng không?"
                duration={3000}
                actionButtons={[
                  {
                    label: "Đồng ý",
